...
<div class="<?php echo $col;?> vz-tv" style="font-family: <?php echo $vz->font?>">
  ...
  <div class="col-md-12 vid-description">
    <div class="row">            
        <div class="col-sm-12">
            <?php if($config->title){ ?><h1 class="video-title"><?php echo $vid->title;?></h1><?php }?>
        </div>
        <?php if($config->longdesc){ ?><div class="col-md-12 desc"><?php echo $vid->longdesc; ?></div><?php }?>
        ...
        <div class="advertise">
            <?php if($config->showad){ if($config->leaderboard){?><div class="col-md-12"><?php echo $config->leaderboard; ?></div><?php }}?>
        </div>
        ...
        <div class="pull-right col-sm-9 text-right">
            <a href="<?php echo $_SESSION['user']->Authenticated ?  '#feed-foot' : '/account/login?redirect='.urlencode($_SERVER['REQUEST_URI'])?>" <?php echo $_SESSION['user']->Authenticated ?  'data-toggle="collapse"' : ''?>><span><i class="fa fa-comment"></i> Comments <?php echo $vz->commentsizelabel;?></span></a>
            ...
            <?php }?>    
            <?php if($config->share){ ?><a href="#share-box" data-toggle="collapse"><span><i class="fa fa-share-square icons"></i> Share</span></a>
            <a href="<?php echo $_SESSION['user']->Authenticated ?  '#email-box' : '/account/login?redirect='.urlencode($_SERVER['REQUEST_URI'])?>" <?php echo $_SESSION['user']->Authenticated ?  'data-toggle="collapse"' : ''?>><i class="fa fa-file icons"></i> Email</a><?php }?>
            <?php if($config->addtolist){ ?><?php if(isset($vz->user->ID)){ ?>
            <a href="<?php echo $_SESSION['user']->Authenticated ?  '#add-to-list' : '/account/login?redirect='.urlencode($_SERVER['REQUEST_URI'])?>" <?php echo $_SESSION['user']->Authenticated ?  'data-toggle="collapse"' : ''?>><span><i class="fa fa-plus-circle icons"></i> Add To</span></a>
            <?php }}?>
        </div>
    </div>
    ...
    <div id="add-to-list" class="collapse row">
        <div class="col-sm-12">           
            <div class="well-gray">
                <ul class="add-to-list list-unstyled"  data-video="<?php echo $vid->sources[0]->id;?>" data-content="<?php echo $vid->mediaid?>">
        ...
        <div class="col-sm-12">
            <form role="form" class="form-inline">
                ...
                <button type="button" id="btn-create-playlist" data-video="<?php echo $vid->sources[0]->id;?>" data-loading-text="creating..." class="btn btn-primary btn-sm">Create Playlist</button>               
                <a href="/account/videomanager" class="btn btn-primary btn-sm">Manage Playlist</a>
            </form>
        </div>
    </div>
    <?php }?>
...
<?php } 
    $vz->Scripts=array(
        ...
        //"//live.vzconsole.com/vz/player2?v=FU0cnc_TK-1SYGWRF-JbcXV6SvIiFXS73nRFrEjRI5s1",
